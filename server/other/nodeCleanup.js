...
function tryAttachProcessHandler(name, handler)
{
    try
    {
...
function tryUninstallHandler(name, handler)
{
    try
    {
...
export default function install(cleanupHandler, stderrMessages)
{
    if (cleanupHandler) {
    ...
    if (cleanupHandlers === null) {
        ...
        tryAttachProcessHandler('SIGINT', sigintHandler);
        tryAttachProcessHandler('SIGHUP', sighupHandler);
        tryAttachProcessHandler('SIGQUIT', sigquitHandler);
        tryAttachProcessHandler('SIGTERM', sigtermHandler);
        tryAttachProcessHandler('uncaughtException', exceptionHandler);
        tryAttachProcessHandler('exit', exitHandler);
...
{
    if (cleanupHandlers !== null) {
        tryUninstallHandler('SIGINT', sigintHandler);
        tryUninstallHandler('SIGHUP', sighupHandler);
        tryUninstallHandler('SIGQUIT', sigquitHandler);
        tryUninstallHandler('SIGTERM', sigtermHandler);
        tryUninstallHandler('uncaughtException', exceptionHandler);
        tryUninstallHandler('exit', exitHandler);
